...
SELECT CASE WHEN DISTANCE < 1 THEN RTRIM(TO_CHAR(DISTANCE*1000 , 'FM9990D99'),'.')||'m' ELSE DISTANCE||'km' END 거리 , C.* 
FROM (
SELECT ROWNUM no , 
       S.* , DISTNACE_WGS84(35.1535583, 126.8879957, SENIOR_LATITUDE, SENIOR_LONGITUDE) AS DISTANCE
FROM SENIOR S 
order by 2 asc) C
